...
def prime_slot_mutation(representation, num_stages, num_slots):
    ...
    non_prime_indices = [i for i in range(len(mutant)) if i not in prime_indices]  # Get all non-prime slot indices.
    ...
    mutant[prime_idx], mutant[non_prime_idx] = (
        mutant[non_prime_idx],
        mutant[prime_idx]
    )
    return mutant # Return the mutated representation.
